...
class TestController extends Controller
{
    ...
    public function certificate($teacher, $test, $place)
    {
        ...
        $fontPath = Yii::getAlias('@app/frontend/fonts/times.ttf');
        ...
        $directoryPath = Yii::getAlias('@webroot/certificates/') . $teacher->subject->title;
        if (!is_dir($directoryPath)) {
            mkdir($directoryPath, 0755, true);
        }
        $newPath = $directoryPath . '/' . $teacher->name . '.jpeg';
    ...
    public function actionDownloadZip($type, $id){
        ...
        if($type == 'certificates'){
            ...
            $filePaths = ResultPdf::find()
                ->andWhere(['like', 'path', 'journals/%.pdf', false])
                ->select('path')
                ->all();
